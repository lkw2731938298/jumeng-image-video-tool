 ...
 */
export function sniffUploadMime(data: Uint8Array): string | undefined {
  const starts = (...bytes: number[]) => bytes.every((b, i) => data[i] === b)
  const ascii = (offset: number, text: string) =>
    [...text].every((ch, i) => data[offset + i] === ch.charCodeAt(0))
 ...
 */
export function assertUploadPayload(mime: string, bytes: number, label: string): UploadKind {
  if (!UPLOAD_EXT_BY_MIME[mime]) {
    throw new Error(
      `不支持上传的类型 ${mime}（${label}）。图片支持 JPEG/PNG/GIF/WEBP，视频支持 MP4/MOV/WebM。`,
    )
  }
  const kind: UploadKind = mime.startsWith('video/') ? 'video' : 'image'
  if (bytes <= 0) {
  ...
  if (bytes > limit) {
    throw new Error(
      `${kind === 'image' ? '图片' : '视频'}超过上传上限 ${limit / 1024 / 1024}MB（${label}，实际 ${bytes} bytes）`,
    )
  }
  return kind
}
...
export function uploadFilenameFor(mime: string, prefix = 'reference'): string {
 ...
 */
export async function readUploadFromDisk(
  filePath: string,
): Promise<{ data: Uint8Array; filename: string; mime: string; kind: UploadKind }> {
  const abs = path.resolve(filePath)
  let data: Buffer
  try {
 ...
 */
export function readUploadFromBase64(
  raw: string,
  label = 'base64 参考素材',
): { data: Uint8Array; filename: string; mime: string; kind: UploadKind } {
