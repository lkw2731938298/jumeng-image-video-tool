...
import { JumengApiError, JumengClient, isImageModel, isVideoModel, } from './api.js';
import { downloadToFile, guessImageExtFromUrl, guessVideoExtFromUrl, isHttpUrl, looksLikeBase64, readUploadFromBase64, readUploadFromDisk, resolveOutputDir, saveBase64Image, uniqueStamp, } from './files.js';
import { importHarnessPackage } from './harness-import.js';
const { defineTool } = await importHarnessPackage('@deepseek-ai/dsh-tools');
...
export const Config = Schema.object({
    apiKey: Schema.string().role('secret').description('聚梦 API Key 字面量（优先用 Web UI / 凭据，避免写入配置文件）'),
...
function requireModel(explicit, fallback, kind) {
    const model = (explicit || fallback || '').trim();
    if (!model) {
        throw new JumengApiError(`${kind === 'image' ? '生图' : '生视频'}未指定 model。请先调用 list_models，或在插件配置中填写 ${kind === 'image' ? 'imageModel' : 'videoModel'}。`);
    }
    return model;
}
/**
...
async function resolveApiKey(ctx, config) {
    ...
    if (fromEnv)
        return fromEnv;
    throw new JumengApiError(`未配置聚梦 API Key。请打开 设置 → 插件 → 插件配置 → 聚梦媒体，填写 API Key 并保存；或设置环境变量 ${refName}。`);
}
async function createClient(ctx, config) {
 ...
 */
async function resolveReference(client, raw, label, opts) {
 ...
 */
async function resolveReferences(client, raws, label, opts) {
    const urls = [];
    const notes = [];
...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        name: 'list_models',
        description: '列出聚梦 AI（Jumeng）可用模型。生成图片/视频前应先调用本工具选型；可按 kind 过滤 image 或 video。',
        parameters: {
            kind: {
                type: 'string',
        ...
        async execute(args, exec) {
            ...
            let models = all;
            if (kind === 'image')
                models = all.filter(isImageModel);
            else if (kind === 'video')
                models = all.filter(isVideoModel);
            const slim = models.map((m) => ({
                id: m.id,
                supported_endpoint_types: m.supported_endpoint_types || [],
            }));
            const text = `聚梦可用模型（kind=${kind}，共 ${slim.length} 个）\n` + formatModels(models);
            return { kind, count: slim.length, models: slim, text };
        },
    }));
    ctx.tools.register(defineTool({
        name: 'upload_file',
        description: '把本地图片或视频上传到聚梦 AI，换取可作为参考素材的临时 URL（约 1 小时后失效）。generate_image / generate_video 已会自动上传非 URL 参考素材，仅当需要复用同一素材多次时才显式调用。图片支持 JPEG/PNG/GIF/WEBP 最大 10MB，视频支持 MP4/MOV/WebM 最大 100MB。',
        parameters: {
            file: {
                type: 'string',
                required: true,
    ...
    ctx.tools.register(defineTool({
        name: 'generate_image',
        description: '使用聚梦 AI 生成图片（文生图；传入 image 则为图生图）。成功后下载到本地并返回绝对路径与字节大小。Agent 必须立即调用 present 工具，传入该本地绝对路径，Harness UI 才会展示交付物。模型可用 list_models(kind=image) 查询。',
        parameters: {
            ...
            image: {
                type: 'string',
                description: '参考图（图生图，部分模型支持）：可传 http(s) URL、本地文件绝对路径或 base64；非 URL 会先经 /files/upload 上传',
            },
    ...
    ctx.tools.register(defineTool({
        name: 'generate_video',
        description: '使用聚梦 AI 生成视频（文生视频；传入 image/images 则为图生视频）。异步提交并轮询，成功后下载到本地并返回绝对路径与字节大小。Agent 必须立即调用 present 工具，传入该本地绝对路径，Harness UI 才会展示交付物。模型可用 list_models(kind=video) 查询。',
        parameters: {
            prompt: { type: 'string', required: true, description: '提示词' },
            ...
            image: {
                type: 'string',
                description: '单张参考素材（图生视频首帧）：可传 http(s) URL、本地文件绝对路径或 base64；非 URL 会先经 /files/upload 上传',
            },
            images: {
                type: 'array',
                items: { type: 'string' },
